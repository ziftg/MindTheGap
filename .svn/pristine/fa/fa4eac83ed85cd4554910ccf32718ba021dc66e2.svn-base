...
public class TfLOverlayManager implements MapEventsReceiver {
    ...
    public void updateMarkerOfNearest(Station nearest) {
        ...
        for (int i = 0; i < StationManager.getInstance().getNumStations(); i++) {
            Station station = (Station) stnClusterer.getItem(i).getRelatedObject();
            if (nearest != null) {
                if (station.equals(nearest)) {
                    stnClusterer.getItem(i).setIcon(closestStnIconDrawable);
                } else
                    stnClusterer.getItem(i).setIcon(stnIconDrawable);
            } else
                stnClusterer.getItem(i).setIcon(stnIconDrawable);
        }
    }
    ...
    public boolean longPressHelper(GeoPoint geoPoint) {
        double geoLat = geoPoint.getLatitude();
        double getLon = geoPoint.getLongitude();
        for (Station station : StationManager.getInstance()) {
        /*for (int i = 0; i < StationManager.getInstance().getNumStations(); i++) {
            Station station = (Station) stnClusterer.getItem(i).getRelatedObject();*/
            if (geoLat != station.getLocn().getLatitude() && getLon != station.getLocn().getLongitude()) {
                tubeLineOverlays.clear();
                updateOverlays();
            }
        }
    ...
    private void markStations() {
        ...
        for (Station next : StationManager.getInstance()) {
            ...
            stnMarker.setPosition(new GeoPoint(next.getLocn().getLatitude(), next.getLocn().getLongitude()));
            stnMarker.setRelatedObject(next);
            stnClusterer.add(stnMarker);
        }
        stnClusterer.clusterer(mapView);
    }
    ...
    private void plotLines() {
        if (StationManager.getInstance().getSelected() != null) {
            updateOverlays();
            for (Line selectedLine : StationManager.getInstance().getSelected().getLines()) {
                for (Branch branch : selectedLine.getBranches()) {
                    ...
                    polyline.setColor(selectedLine.getColour());
